...
class ModeloPonderadorReporte extends CI_Model
{
 ...
 function getTotalRowAllData()
  {
    $query = $this->db->query("SELECT count(*) as row FROM PonderadoresReportes")->row_array();
    return $query['row'];
  }
...
function data_pagination($url, $rows = 5, $uri = 3)
 ...
 // untuk config class pagination yg lainnya optional (suka2 lu.. :D )
  ...
  {
    $this->db->insert('PonderadoresReportes',$arreglo);
  }

function obtenerFicha ($idPonderador){
        return $this->db->query("SELECT * from PonderadoresReportes where idPonderador=idPonderador ")->row();
    }
  ...
  { 
    ...
    $this->db->update('PonderadoresReportes', $arreglo); 
  ...
  { 
    $this->db->where('idPonderador', $idPonderador);
    $this->db->delete('PonderadoresReportes'); 
    }
