...
class PonderacionesArnes extends CI_Model{
    public $variable;
    ...
    {
        $query = $this->db->query("SELECT count(*) as row FROM ponderadorSshi")->row_array();
        return $query['row'];
    }
    ...
    function getDatos($no_page)
    {
        ...
        return $this->db->query("SELECT ponderadorSshi.* FROM ponderadorSshi")->result_array();
    }

    function insertaDatos($data)
    {
        $this->db->insert('ponderadorSshi', $data);
...
/*    function insertaDatosPuent($data2)
    ...
    function obtenerFicha ($idPonderacion){
        return $this->db->query("SELECT * from ponderadorSshi where idPonderador=$idPonderacion ")->row();
    }
    ...
    function modificaDatos($data,$idPonderacion)
    {
        ...
        $this->db->update('ponderadorSshi', $data);
