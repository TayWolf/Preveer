...
class Entregable extends CI_Model{
    public $variable;
	...
	{
 		$query = $this->db->query("SELECT count(*) as row FROM Entregables")->row_array();
 		return $query['row'];
	}
 ...
 // untuk config class pagination yg lainnya optional (suka2 lu.. :D )
	...
	function insertaDatos($data)
	{
   	...
   	function obtenerFicha ($idEntregable){
        return $this->db->query("SELECT * from Entregables where idEntregable=$idEntregable ")->row();
   	}
  ...
  { 
    $this->db->where('idEntregable', $idInmueble);
    $this->db->update('Entregables', $data); 
