...
class AnalisisRiesgo extends CI_Model{
    ...
    function getTotalRowAllData($area, $usuario)
    {
        if($area==1)
            ...
            $query = $this->db->query("SELECT COUNT(DISTINCT(asignaInmueble.idOti)) as row FROM CentrosDeTrabajo JOIN asignaInmueble ON CentrosDeTrabajo.idCentroTrabajo=asignaInmueble.idCentroTrabajo JOIN AnalistaOti ON AnalistaOti.idAsignacion = asignaInmueble.idAsignacion JOIN Usuario ON AnalistaOti.idUsuario=Usuario.idUsuario JOIN Logeo ON Usuario.idUsuario=Logeo.idUsuario join serviciosSubservicios on serviciosSubservicios.idControl=asignaInmueble.idProyecto join Subservicios on Subservicios.idSubservicio=serviciosSubservicios.idSubservicio JOIN Formato ON Formato.idFormato = CentrosDeTrabajo.idFormato WHERE Logeo.idUsuario=$usuario")->row_array();
        return $query['row'];
    }
    ...
    function getDatosAnalista($no_page, $usuario)
    {
        ...
        return $this->db->query("SELECT Subservicios.idSubservicio, asignaInmueble.idProyecto, CONCAT(Formato.nombre, ' (OTI ', asignaInmueble.idOti, ')') as nombreFormato, CentrosDeTrabajo.*, asignaInmueble.idAsignacion, asignaInmueble.idOti FROM CentrosDeTrabajo JOIN asignaInmueble ON CentrosDeTrabajo.idCentroTrabajo = asignaInmueble.idCentroTrabajo JOIN AnalistaOti ON AnalistaOti.idAsignacion = asignaInmueble.idAsignacion JOIN Usuario ON AnalistaOti.idUsuario = Usuario.idUsuario JOIN Logeo ON Usuario.idUsuario = Logeo.idUsuario join serviciosSubservicios on serviciosSubservicios.idControl = asignaInmueble.idProyecto join Subservicios on Subservicios.idSubservicio = serviciosSubservicios.idSubservicio JOIN Formato ON Formato.idFormato = CentrosDeTrabajo.idFormato JOIN Oti ON asignaInmueble.idOti = Oti.idOti WHERE Logeo.idUsuario = $usuario AND Oti.statusActiva=1 GROUP BY asignaInmueble.idOti")->result_array();
    }
...
function insertarArregloRevisionInstalaciones($arrayResiduosPeligrosos)
{
   ...
   return $this->db->query('SELECT LAST_INSERT_ID()')->result_array();
    ...
    {
        return $this->db->query("SELECT HidraulicaCatalogoPuente.*, CatalogoHidraulica.nombre,InstalacionesHidraulicas.idAsignacion FROM InstalacionesHidraulicas 
                                JOIN asignaInmueble ON InstalacionesHidraulicas.idAsignacion = asignaInmueble.idAsignacion  
                                JOIN HidraulicaCatalogoPuente ON InstalacionesHidraulicas.idInstalacionesHidraulicas = HidraulicaCatalogoPuente.idCatalogo 
                                JOIN CatalogoHidraulica ON HidraulicaCatalogoPuente.idInstalacion = CatalogoHidraulica.idCatalogoHidraulica 
                                WHERE asignaInmueble.idAsignacion = $idAsignacion")->result_array();
    }
