...
class ModelPrioridadMejora extends CI_Model
{
 ...
 function getTotalRowAllData()
	{
 		$query = $this->db->query("SELECT count(*) as row FROM prioridadMejora")->row_array();
 		return $query['row'];
	}
...
function data_pagination($url, $rows = 5, $uri = 3)
 ...
 // untuk config class pagination yg lainnya optional (suka2 lu.. :D )
	...
	function getDatos($no_page)
	{
		...
		return $this->db->query("SELECT * FROM prioridadMejora")->result_array();
	}


	function insertaDatos($arreglo)
	{
		$this->db->insert('prioridadMejora',$arreglo);
	}

function obtenerFicha ($idPrioridad){
        return $this->db->query("SELECT * from prioridadMejora where idPrioridad=idPrioridad ")->row();
   	}
  ...
  { 
    ...
    $this->db->update('prioridadMejora', $arreglo); 
  ...
  { 
    $this->db->where('idPrioridad', $idPrioridad);
    $this->db->delete('prioridadMejora'); 
    }
