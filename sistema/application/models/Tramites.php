...
class Tramites extends CI_Model{
  ...
  function getTotalRowAllData()
	{
 		$query = $this->db->query("SELECT count(*) as row FROM Tramites")->row_array();
 		return $query['row'];
	}
 ...
 // untuk config class pagination yg lainnya optional (suka2 lu.. :D )
	...
	function insertaDatos($data)
	{
   	...
   	function obtenerFicha ($idTramite){
        return $this->db->query("SELECT * from Tramites where idTramite=$idTramite ")->row();
   	}
    ...
    { 
      ...
      function modificaDatos($data,$idTramite)
  { 
    $this->db->where('idTramite', $idTramite);
    $this->db->update('Tramites', $data); 
  ...
  { 
    ...
    function borrarDatos($idTramite)
  { 
    $this->db->where('idTramite', $idTramite);
