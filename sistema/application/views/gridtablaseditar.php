...
<script type="text/javascript">
  ...
  $("#form").on("submit", function(e){
    ...
    url= "<?php echo 'https://cointic.com.mx/preveer/sistema/index.php/CrudPlantillas/editarTablaPlantilla/';?>";
    e.preventDefault();
    var f = $(this);
        ...
        <div class="container-fluid">
            ...
            <div class="row clearfix">
                ...
                <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                    ...
                    <div class="card">
                        <div class="header">
                            <h2>Modifique los datos de la tabla </h2>
...
function getIndicador(x)
    ...
    function agregarColumnaBD(x,idI,valorColumna)
{
                            ...
                            if (data.length>0)
                             {
                                ...
                                for (ie=0; ie< data.length; ie++) {
                                  ...
                                  $("#tabla"+x+"indicad"+conIndicador).append(new Option(data[ie]['nombreIndicador'],data[ie]['idIndicador']));
                                    ...
                                    $("#tabla"+x+"indicad"+conIndicador).val(idI);
                                    $("#tabla"+x+"NombreColumna"+conIndicador).val(valorColumna);
...
function contadores()
{
  ...
  var totalT=0;
  $.ajax({
                        url : "<?php echo site_url('CrudPlantillas/contadores/')?>" + idPlantilla,
                        type: "post",
                        dataType: "JSON",
                        success: function(data)
                        {
                          ...
                          for (var i = 0; i < data.length; i++) {
                            agregarTableBD(data[i]["idPlantillaTabla"]);
                            getformularioAcordeon(i,data[i]["idFormulario"],data[i]["idAcordeon"],data[i]["nombreTabla"]);
...
function getformularioAcordeon(i,idF,idA,nTabla)
{

  var idTabla=$("#idTabla"+i).val();
  $("#nombreTabla"+i).val(nTabla);
 $.ajax({
                        url : "<?php echo site_url('CrudPlantillas/getForm/')?>" ,
                        type: "post",
                        dataType: "JSON",
                        success: function(data)
                        {
                           ...
                           $("#idFormm"+i).append('<option value ="">Seleccione una opción</option>');
                            if (data.length>0)
                             {
                                for (ie=0; ie< data.length; ie++) {
                                  $("#idFormm"+i).append(new Option(data[ie]['nombreFormulario'],data[ie]['idControl']));
                                    }
                                    $("#idFormm"+i).val(idF);      
                            }
                          ...
                          $.ajax({
                            url : "<?php echo site_url('CrudPlantillas/getCordeones/')?>" + idF,
                            type: "post",
                            dataType: "JSON",
                            success: function(data)
                            {
...
function contadoresInternos(idTabla,x)
{
  
  $.ajax({
                        url : "<?php echo site_url('CrudPlantillas/contadoresInter/')?>" + idTabla,
                        type: "post",
                        dataType: "JSON",
                        success: function(data)
                        {
                          for (var i = 0; i < data.length; i++) {
                           // alert(i)
                           agregarColumnaBD(x,data[i]["idIndicador"],data[i]["nombreColumna"]);
                          }
                        },
                        error: function (jqXHR, textStatus, errorThrown)
                        {
                           ...
                           '<div class="row">'+
                               '<div class="col-md-4">'+
                                    '<div class="form-group">'+
                                        '<div class="form-line">'+
                                            '<label for="nickUser">Nombre de la tabla</label>'+
                                            '<input type="hidden" id="idTabla'+conTabla+'"  value="'+idT+'">'+
                                                '<input type="text" form="form" class="form-control" id="nombreTabla'+conTabla+'" name="nombreTabla'+conTabla+'" placeholder="Ingrese el nombre de la tabla"  required />'+
                                        '</div>'+
                                    '</div> '+  
                               '</div>'+
