    ...
    <div class="row clearfix">
        <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
            <div class="card">
                <div class="header">
                    <h2>
                        FECHAS PROGRAMADAS PARA REVISIÓN DE DOCUMENTOS
                ...
                <div class="body table-responsive">
                    <table class="table table-hover" >
                        ...
                        $consulta=$conexion->query("SELECT asignaInmueble.idAsignacion, CentrosDeTrabajo.nombre FROM asignaInmueble JOIN CentrosDeTrabajo ON CentrosDeTrabajo.idCentroTrabajo=asignaInmueble.idCentroTrabajo JOIN AnalistaOti ON AnalistaOti.idAsignacion=asignaInmueble.idAsignacion WHERE asignaInmueble.idAsignacion IN (SELECT VisitasInmueble.idAsignacion FROM VisitasInmueble WHERE VisitasInmueble.tipoVisita=2 ) and AnalistaOti.idUsuario=$usuario");
                        //$datos=array();
                        $contador=1;
...
<div class="modal fade" id="myModalfechaini" role="dialog">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal">&times;</button>
                <h4 class="modal-title">Por favor indique la fecha de revisión. <p id="nombreTitul"></p></h4>
            </div>
...
<script type="text/javascript">
    ...
    function  identi(id)
    {
        ...
        var parametro={"id":id}
        $.ajax({
            url : "consultafechasdocumentos.php",
            type: "POST",
            data: parametro,
            dataType: "JSON",
            success: function(data)
            {
                if (data.length>0)
                {
                    ...
                    for (i=0; i<data.length; i++)
                    {
                        ...
                        $("#listadoVis").append('<tr><td style="display:none">'+data[i]['idVisita']+'</td><td scope="row">'+ii+'</td><td>'+data[i]['fechaAgenda']+'</td><td>'+data[i]['comentario']+'</td></tr>');

                        ii++;

                    }
                    $('#tablaListado').Tabledit({
                        url: 'modificavisita.php',
                        editButton: false,
                        deleteButton:false,
                        columns: {
                            identifier: [0, 'idIdentif'],
                            editable: [[2, 'fechPr'],[3, 'comentE']]
                        }
                    });
                    $("input[name*='fechPr']").attr("type",'date');
