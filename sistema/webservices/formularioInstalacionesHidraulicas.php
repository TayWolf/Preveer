            ...
            <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                <div class="card">
                    ...
                    <div class="body">
                        ...
                        $existencia=$conexion->query("SELECT InstalacionesHidraulicas.* FROM InstalacionesHidraulicas JOIN asignaInmueble ON InstalacionesHidraulicas.idAsignacion=asignaInmueble.idAsignacion WHERE asignaInmueble.idAsignacion = $idAsignacion");
                        $catalogo=$conexion->query("SELECT CatalogoHidraulica.* FROM CatalogoHidraulica");
                        $contador=0;
                        $row=array('idInstalacionesHidraulicas'=>"", 'suministro'=>"", 'sumOtro'=>"", 'tuberia'=>"", 'idAsignacion'=>"" );
                        foreach ($existencia as $ro)
                        {
                            //$row=$ro;
                            $contador++;
                        }
                        if($contador==0)
                        {
                            $insercion=$conexion->prepare("INSERT INTO InstalacionesHidraulicas(idAsignacion) VALUES (?)");
                            $insercion->bindParam(1,$idAsignacion);
                            $insercion->execute();
                            $existencia=$conexion->query("SELECT InstalacionesHidraulicas.* FROM InstalacionesHidraulicas JOIN asignaInmueble ON InstalacionesHidraulicas.idAsignacion=asignaInmueble.idAsignacion WHERE asignaInmueble.idAsignacion = $idAsignacion");
                        }
                        $hidraulicaPuente=$conexion->query("SELECT HidraulicaCatalogoPuente.*, CatalogoHidraulica.nombre,InstalacionesHidraulicas.idAsignacion FROM InstalacionesHidraulicas JOIN asignaInmueble ON InstalacionesHidraulicas.idAsignacion = asignaInmueble.idAsignacion JOIN HidraulicaCatalogoPuente ON InstalacionesHidraulicas.idInstalacionesHidraulicas = HidraulicaCatalogoPuente.idCatalogo JOIN CatalogoHidraulica ON HidraulicaCatalogoPuente.idInstalacion = CatalogoHidraulica.idCatalogoHidraulica WHERE HidraulicaCatalogoPuente.idAsignacion=$idAsignacion");
                                            ...
                                            <div class="row">
                                                <div class="col-sm-6">
                                                    <div class="form-group">
                                                        <div class="form-line">
                                                            <b>Instalación</b>
                                                            <select type="number" class="form-control" id="instalacion" name="instalacion" min="0" >
                                                                <option value="">Seleccione una opción</option>
                                                                <?php
...
<script type="text/javascript">
    ...
    function AgregarTipoInsHidraulica()
    {
        ...
        $.ajax(
            {
                url: "https://cointic.com.mx/preveer/sistema/index.php/CrudAnalisisRiesgo/insertarArregloInstalacionesHidraulicas/" + <?=$idAsignacion?>,
                type: "post",
                dataType: "html",
                data: formData,
                cache: false,
...
if($tipo!='' && $idusuariobase!= '')
{
    ...
    if($tipo == 1){ //Menu flotante para administrador
        ...
        <li class='mfb-component__wrap'>
            ...
            <ul class='mfb-component__list'>
            <li>
                <a href=".$site_url.('CrudOti/coordinador')."/".$idUsuarioBase." data-mfb-label='OTI' class='mfb-component__button--child'>
                <i class='material-icons'>playlist_add_check</i>
                </a>
            </li>
            </ul>
